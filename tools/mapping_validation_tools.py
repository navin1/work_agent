...
def _deconstruct_sql(sql: str) -> dict:
    ...
    try:
        ...
        # GROUP BY — last one = outermost query
        groups = list(tree.find_all(exp.Group))
        if groups:
            base["group_by"] = [e.sql(dialect="bigquery") for e in groups[-1].expressions]

        # SELECT expressions — last SELECT = main query output
        selects = list(tree.find_all(exp.Select))
        if selects:
            for expr in selects[-1].expressions:
                alias = (
                    expr.alias
...
def _cache_key(rule_text: str, clauses_str: str) -> str:
    return hashlib.sha256(f"{rule_text}|||{clauses_str}".encode()).hexdigest()
...
def _step_a_classify(rule: dict, structure: dict) -> dict:
    """Step A: identify which SQL sections are relevant for this rule."""
    cte_lines = "\n".join(
        f"  {n}: {s[:150]}..." if len(s) > 150 else f"  {n}: {s}"
        for n, s in list(structure.get("ctes", {}).items())[:8]
    ) or "  (none)"
    join_lines = "\n".join(
        f"  {j['type']} JOIN {j['table']} ON {j['on']}"
        for j in structure.get("joins", [])[:6]
    ) or "  (none)"
    where_lines = "\n".join(f"  {w}" for w in structure.get("where_clauses", [])[:4]) or "  (none)"
    select_lines = "\n".join(
        f"  {a}: {e}" for a, e in list(structure.get("select_expressions", {}).items())[:15]
    ) or "  (none)"

    prompt = (
        "Data QA task: identify which SQL sections are relevant to verify a business rule.\n\n"
        f"RULE:\n"
        f"  Target: {rule.get('target_columns', [])}\n"
        f"  Source: {rule.get('source_columns', [])}\n"
        f"  Text: {rule.get('rule_text', '')}\n\n"
        f"SQL ELEMENTS:\n"
        f"CTEs:\n{cte_lines}\n"
        f"JOINs:\n{join_lines}\n"
        f"WHERE:\n{where_lines}\n"
        f"SELECT:\n{select_lines}\n"
        f"GROUP BY: {structure.get('group_by', [])}\n"
        f"Aggregations: {structure.get('aggregations', [])[:8]}\n\n"
        'Return ONLY valid JSON (no markdown):\n'
        '{"relevant_ctes": [], "relevant_clauses": ["join","where","group_by","aggregation","select"], "target_expressions": []}'
    )
    try:
        result = _extract_json(_call_llm(prompt))
        if result and isinstance(result.get("relevant_ctes"), list):
            return result
    except Exception:
        pass
    return {
        "relevant_ctes": list(structure.get("ctes", {}).keys())[:3],
        "relevant_clauses": ["join", "where", "select", "aggregation"],
        "target_expressions": rule.get("target_columns", []),
    }


def _step_b_verify(rule: dict, structure: dict, classification: dict) -> dict:
    """Step B: verify whether the relevant SQL correctly implements the rule."""
    relevant_ctes = classification.get("relevant_ctes", [])
    relevant_clauses = set(classification.get("relevant_clauses", []))
    parts = []

    for name in relevant_ctes:
        sql = structure.get("ctes", {}).get(name, "")
        if sql:
            parts.append(f"CTE [{name}]:\n{sql[:700]}")

    if "join" in relevant_clauses and structure.get("joins"):
        parts.append(
            "JOINs:\n"
            + "\n".join(f"  {j['type']} JOIN {j['table']} ON {j['on']}" for j in structure["joins"])
        )
    if "where" in relevant_clauses and structure.get("where_clauses"):
        parts.append("WHERE:\n" + "\n".join(structure["where_clauses"]))
    if "group_by" in relevant_clauses and structure.get("group_by"):
        parts.append("GROUP BY: " + ", ".join(structure["group_by"]))
    if "aggregation" in relevant_clauses and structure.get("aggregations"):
        parts.append("Aggregations:\n" + "\n".join(structure["aggregations"][:10]))

    # Target column SELECT expressions — prefer exact or partial name match
    target_exprs: dict[str, str] = {}
    for alias, expr in structure.get("select_expressions", {}).items():
        for t in rule.get("target_columns", []):
            if _norm(t) in _norm(alias) or _norm(alias) in _norm(t):
                target_exprs[alias] = expr
    if not target_exprs:
        target_exprs = dict(list(structure.get("select_expressions", {}).items())[:8])
    if target_exprs:
        parts.append(
            "SELECT (target column expressions):\n"
            + "\n".join(f"  {a}: {e}" for a, e in target_exprs.items())
        )

    context = "\n\n".join(parts) or "(no SQL extracted for this rule)"
    prompt = (
        "You are a senior Data QA Engineer. Verify if SQL correctly implements a business rule.\n\n"
        f"BUSINESS RULE:\n"
        f"  Target: {rule.get('target_columns', [])}\n"
        f"  Source: {rule.get('source_columns', [])}\n"
        f"  Rule: {rule.get('rule_text', '')}\n\n"
        f"RELEVANT SQL:\n{context}\n\n"
        "Evaluate: JOIN type, WHERE/filter conditions, aggregation function, group-by columns, "
        "expression logic, NULL handling.\n\n"
        'Return ONLY valid JSON (no markdown):\n'
        '{"verdict": "PASS|FAIL|PARTIAL", "reason": "1-2 sentences", "evidence": "specific SQL snippet", "flags": []}'
    )
    try:
        result = _extract_json(_call_llm(prompt))
        if result and result.get("verdict") in ("PASS", "FAIL", "PARTIAL"):
            return result
    except Exception:
        pass
    return {
        "verdict": "PARTIAL",
        "reason": "LLM evaluation inconclusive. Manual review required.",
        "evidence": "",
        "flags": ["LLM evaluation error"],
    }


def _evaluate_rule(rule: dict, structure: dict, force_refresh: bool) -> dict:
    """Two-step LLM evaluation with L1 (session) + L2 (file) caching."""
    clauses_str = json.dumps(
        {
            "joins":  structure.get("joins", []),
            "where":  structure.get("where_clauses", []),
            "group":  structure.get("group_by", []),
            "select": list(structure.get("select_expressions", {}).keys()),
            "aggs":   structure.get("aggregations", []),
        },
        sort_keys=True,
    )
    key = _cache_key(rule.get("rule_text", ""), clauses_str)

    if not force_refresh:
        cached = _get_cached_verdict(key)
        if cached:
            return {**cached, "cache_hit": True}

    classification = _step_a_classify(rule, structure)
    verdict_data   = _step_b_verify(rule, structure, classification)

    result = {
        "verdict":          verdict_data.get("verdict", "PARTIAL"),
        "reason":           verdict_data.get("reason", ""),
        "evidence":         verdict_data.get("evidence", ""),
        "flags":            verdict_data.get("flags", []),
        "relevant_ctes":    classification.get("relevant_ctes", []),
        "relevant_clauses": classification.get("relevant_clauses", []),
        "cache_hit":        False,
    }
    _save_verdict(key, result)
    return result
...
def validate_mapping_rules(
    ...
    mapping_file_name: Excel file name (with or without .xlsx/.xls).
    ...
    try:
        ...
        try:
            ...
            return safe_json({"error": f"DuckDB query failed: {exc}", "table_name": table_name})

        if df.empty:
            return safe_json({"error": f"Table {table_name} is empty.", "table_name": table_name})
        ...
        raw_rules: list[dict] = []
        for rule_id, ((bq_val, _key_val), grp) in enumerate(group_map.items(), start=1):
        ...
        for bq_label, group_rules in bq_groups.items():
            ...
            for rule in group_rules:
                ...
                if rule["_na"]:
                    ...
                    try:
                        verdict_data = _evaluate_rule(rule, structure, force_refresh)
